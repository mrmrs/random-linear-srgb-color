...
test('returns a CSS color() string with two-decimal channels and no alpha by default', (t) => {
  for (let i = 0; i < 20; i++) {
    ...
    t.regex(color, /^color\(linear-srgb (?:0\.\d{2}|1\.00) (?:0\.\d{2}|1\.00) (?:0\.\d{2}|1\.00)\)$/);
  }
});

test.serial('keeps integer percentage ranges within bounds', (t) => {
  withRandomValues([0.73, 0.21, 0.45], () => {
    t.is(randomLinearSRGBColor(0, 100, 0, 100, 0, 100), 'color(linear-srgb 73% 21% 45%)');
  });
});

test.serial('keeps decimal percentage ranges to two decimal places', (t) => {
  withRandomValues([0.5, 0.5, 0.5], () => {
    t.is(randomLinearSRGBColor(1.5, 1.5, 2.25, 2.25, 10.5, 10.5), 'color(linear-srgb 1.50% 2.25% 10.50%)');
  });
});

test.serial('omits alpha when no alpha range is supplied', (t) => {
  withRandomValues([0.1, 0.2, 0.3], () => {
    t.is(randomLinearSRGBColor(), 'color(linear-srgb 0.10 0.20 0.30)');
  });

  withRandomValues([0.1, 0.2, 0.3, 0.4], () => {
    t.is(randomLinearSRGBColor(0, 1, 0, 1, 0, 1, 0, 1), 'color(linear-srgb 0.10 0.20 0.30 / 0.40)');
  });
});

test('treats undefined alpha arguments as omission', (t) => {
  const result = randomLinearSRGBColor(0, 1, 0, 1, 0, 1, undefined, undefined);
  t.notRegex(result, / \/ /);
});
...
test('omits alpha from object output when not requested', (t) => {
  const color = randomLinearSRGBColor(0, 1, 0, 1, 0, 1, undefined, undefined, true);
...
test.serial('validates ranges before generating any random values', (t) => {
  ...
  try {
    t.throws(() => randomLinearSRGBColor(0, 1, 0, 1, 1, 0), {
...
test('validates range values', (t) => {
  t.throws(() => randomLinearSRGBColor(Number.NaN), {
    instanceOf: TypeError,
    message: 'red range must use finite numbers'
  });

  t.throws(() => randomLinearSRGBColor(1, 0), {
    instanceOf: RangeError,
    message: 'red minimum must be less than or equal to maximum'
  });

  t.throws(() => randomLinearSRGBColor(0, 1, 0, 1, 0, 1, 2, 1), {
    instanceOf: RangeError,
    message: 'alpha minimum must be less than or equal to maximum'
  });

  t.throws(() => randomLinearSRGBColor(0, 1, 0, 1, 0, 1, -0.1, 1), {
    instanceOf: RangeError,
    message: 'alpha range must be between 0 and 100'
  });

  t.throws(() => randomLinearSRGBColor(0, 1, 0, 1, 0, 1, 0, 100.1), {
    instanceOf: RangeError,
    message: 'alpha range must be between 0 and 100'
  });
});
